...
file_input = "/media/sreeganesh/Windows/Users/GMachine/Documents/Studies/S7/NTC/NTC_Assignment/Vigenere/cryptanalysis/chosenPT/input.txt"
file_output = "/media/sreeganesh/Windows/Users/GMachine/Documents/Studies/S7/NTC/NTC_Assignment/Vigenere/cryptanalysis/chosenPT/output.txt"
...
def main():
    with open (file_input, 'rt') as myfile:
        for line in myfile:
            if line.lower().find("message") != -1:    # if case-insensitive match,
                message = line.rstrip('\n').split(" = ")[1]
            elif line.find("plain_text") != -1:    
                plain_text = (line.rstrip('\n').split(" = ")[1])
            elif line.find("cipher_text1") != -1:
                cipher_text1 = (line.rstrip('\n').split(" = ")[1])
            elif line.find("cipher_text2") != -1:
                cipher_text2 = (line.rstrip('\n').split(" = ")[1])
    
    cipher_texts = [cipher_text1, cipher_text2]
    ...
    for i in range(1,len(plain_text)+1):
        for cipher_text in cipher_texts:
            ...
            print("decryption of message: {} for ciphertext: {} and key: {}".format(decrypt(key1,message),cipher_text,key))
            fout.write("decryption of message: {} for ciphertext: {} and key: {}\n".format(decrypt(key1,message),cipher_text,key))
